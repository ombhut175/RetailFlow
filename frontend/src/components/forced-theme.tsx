...
export function ForcedTheme({ theme, mode, children, className = "" }: ForcedThemeProps) {
  ...
  if (!mounted) {
    return (
      <div className={`min-h-[600px] bg-muted/20 animate-pulse rounded-lg ${className}`}>
  ...
  return (
    <div className={`${combinedClasses} ${className}`} data-theme={theme} data-mode={mode}>
      {children}
    </div>
  )
}
