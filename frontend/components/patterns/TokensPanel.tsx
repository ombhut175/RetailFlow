...
export function TokensPanel({ theme, mode, tokens, className = "" }: TokensPanelProps) {
  ...
  const getRadixScale = (tokenName: string, themeKey: string, mode: "light" | "dark") => {
    const radixMappings: Record<string, Record<string, { scale: string; step: string }>> = {
      fintech: {
        "--primary": { scale: "jade", step: mode === "light" ? "9" : "8" },
        "--secondary": { scale: "jade", step: mode === "light" ? "3" : "4" },
        "--accent": { scale: "emerald", step: mode === "light" ? "6" : "7" },
        "--background": { scale: mode === "light" ? "white" : "gray", step: mode === "light" ? "" : "1" },
    ...
    return mapping ? `${mapping.scale}${mapping.step ? `-${mapping.step}` : ""}` : "gray-6"
  }

  return (
    <TooltipProvider>
      <Card className={`${className} border-0 shadow-lg bg-card/50 backdrop-blur-sm`} data-testid="tokens-panel">
        ...
        <CardContent className="space-y-8">
          {Object.entries(tokenCategories).map(([category, { icon, tokens: categoryTokens }]) => (
            <div key={category} className="space-y-4">
              ...
              <div className="grid gap-3">
                {Object.entries(categoryTokens).map(([tokenName, description]) => {
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    ...
                                    onClick={() => copyToClipboard(tokenName, tokenName)}
                                  >
                                    <Copy className="w-3 h-3" />
                                  </Button>
                                </TooltipTrigger>
                                <TooltipContent>
                                  {copiedToken === tokenName ? "Copied!" : "Copy token name"}
                                </TooltipContent>
                              </Tooltip>
                            </div>
