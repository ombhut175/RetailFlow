...
import { CheckCircle2, Lightbulb, Palette, Users, Zap, Target, Eye, Code2 } from "lucide-react"
...
export function Guidance({ guidance, theme, className = "" }: GuidanceProps) {
  const getThemeIcon = (theme: string) => {
    ...
    return icons[theme as keyof typeof icons] || <Palette className="w-4 h-4" />
  }
  ...
  const getThemePhilosophy = (theme: string) => {
    const philosophies = {
      fintech: {
        ...
        description: "Financial interfaces prioritize clarity, security indicators, and data-driven decision making.",
      ...
      edtech: {
        ...
        description: "Educational interfaces focus on progress visualization, interactive learning, and motivation.",
      ...
      saas: {
        ...
        description: "SaaS interfaces prioritize workflow efficiency, data density, and power-user functionality.",
        color: "text-blue-600 dark:text-blue-400",
        bg: "bg-blue-50 dark:bg-blue-950/30",
      },
      social: {
        title: "Connection & Expression",
        description: "Social interfaces emphasize personal expression, community building, and engaging interactions.",
        color: "text-pink-600 dark:text-pink-400",
        bg: "bg-pink-50 dark:bg-pink-950/30",
      },
    }
    return philosophies[theme as keyof typeof philosophies] || philosophies.saas
  }
  ...
  return (
    <Card className={`${className} border-0 shadow-lg bg-card/50 backdrop-blur-sm`} data-testid="guidance">
      <CardHeader className="pb-6">
        <CardTitle className="flex items-center gap-3">
          <div className="p-1.5 bg-primary/10 rounded-md">{getThemeIcon(theme)}</div>
          <span>Design Guidance</span>
          <Badge variant="outline" className="ml-auto capitalize bg-background/50">
            {theme}
          </Badge>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-8">
        {/* Philosophy Section */}
        <div className={`p-6 rounded-xl ${philosophy.bg} border border-current/10`}>
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Lightbulb className={`w-5 h-5 ${philosophy.color}`} />
              <h3 className={`font-semibold ${philosophy.color}`}>{philosophy.title}</h3>
            </div>
            <p className="text-sm leading-relaxed text-muted-foreground">{philosophy.description}</p>
          </div>
        </div>
        ...
        {Object.entries(categorizedGuidance).map(([category, items]) => {
          ...
          return (
            <div key={category} className="space-y-4">
              <div className="flex items-center gap-2">
                <div className="text-muted-foreground">{categoryIcons[category as keyof typeof categoryIcons]}</div>
                  ...
                  <div key={index} className="group">
                    <div className="flex items-start space-x-4 p-4 rounded-lg border bg-background/30 hover:bg-background/50 transition-all duration-200 hover:shadow-sm">
                      ...
                      <div className="space-y-1 flex-1">
                        <span className="text-sm leading-relaxed text-foreground font-medium">{item}</span>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="p-4 bg-muted/30 rounded-lg border border-dashed">
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              <h4 className="text-sm font-medium text-foreground">Implementation Notes</h4>
              <p className="text-xs text-muted-foreground leading-relaxed">
                These guidelines are implemented through semantic CSS variables that automatically adapt to the {theme}{" "}
                theme. All interactive states, color combinations, and accessibility features follow these principles.
              </p>
            </div>
          </div>
        </div>
